...
class AdaptiveSoftmax(nn.Module):
    ...
    def __init__(self, vocab_size, input_dim, cutoff, dropout, factor=4., adaptive_inputs=None, tie_proj=False, padding_idx=-100):
        super().__init__()
    ...
    def forward(self, input, target):
        ...
        assert len(output) == len(new_target)
        # print([item.size() if item is not None else 'None' for item in output])
        # print([item.size() if item is not None else 'None' for item in new_target])

        loss = input.new(1).zero_()

        for i in range(len(new_target)):
            if new_target[i] is not None:
                assert (new_target[i].min() >= 0 and new_target[i].max() <= output[i].size(1))
                # print(f'it {i}')
                # print(output[i].size())
                # print(new_target[i].size())
