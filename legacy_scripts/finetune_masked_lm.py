...
from sent_to_vec.finetune import DoubleClassificationHead, DoubleClassificationWrapper
...
if __name__ == '__main__':
    ...
    FINETUNE_PATH = 'masked-lm-finetune-checkpoint.bin'

    if path.exists(FINETUNE_PATH):
        print('Resuming from finetune checkpoint')
        model = DoubleClassificationWrapper(from_fp=FINETUNE_PATH)
    elif path.exists(MODEL_PATH):
        print('Resuming from saved checkpoint')
        model = BiLanguageModelWrapper(from_fp=MODEL_PATH)
        finetune_model = DoubleClassificationWrapper()
        finetune_model.set_encoder(model)
    ...
    if path.exists(SAVE_PATH):
        ...
        dataset.load(SAVE_PATH, model, get_next_sent=True)
    else:
        paths = [
        ...
        dataset.initialize(model, data_path=paths, get_next_sent=True)
        dataset.save()
